...
function Section({ title, children, extra }: { title: React.ReactNode; children: React.ReactNode; extra?: React.ReactNode }) {
  ...
  return (
    <div className="mb-3">
      ...
      {open && <div className="rounded-lg border border-border bg-card p-2">{children}</div>}
    </div>
  )
}
...
function useOpenFile() {
  ...
  return (f: FileEntry) =>
    navigate(`/f?s=${encodeURIComponent(f.s)}&r=${encodeURIComponent(f.r)}`)
}
...
function orderGroups<T>(catalog: Catalog, groups: Map<string, T[]>) {
  return [...groups.entries()].sort((a, b) => {
    ...
    return (Number(!!A?.project) - Number(!!B?.project)) ||
...
function SourceSection({ catalog, id, children }: { catalog: Catalog; id: string; children: React.ReactNode }) {
  ...
  return (
    <Section title={source?.project ? <>Projeto · <b className="normal-case text-foreground">{source.label}</b></> : <SourceBadge source={source} />}>
      {children}
    </Section>
  )
}
...
export function CategoryView({ cat }: { cat: string }) {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{files.length} arquivo(s)</p>
...
export function SkillsView() {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{catalog.skills.length} skills — todos os arquivos dos diretórios</p>
      {[...groups.entries()].map(([sid, list]) => {
        ...
        for (const skill of list as SkillEntry[]) {
          ...
          for (const f of sourceFiles) {
            ...
            entries.push(f.r === skill.r
              ? { f, label: skill.skill_name }
              : { f })
          }
        }
        return (
...
export function FilesView() {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{catalog.files.length} arquivos indexados</p>
...
export function ProjectsView() {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{catalog.projects.length} projeto(s) com configuração de IA</p>
...
export function DocsView() {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{files.length} arquivo(s) de documentação (pastas docs/ globais e dos projetos)</p>
...
export function ToolsView() {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-muted-foreground">Escolha a ferramenta para ver as configurações globais e por projeto</p>
...
export function McpsView() {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{catalog.mcps.length} servidores configurados</p>
      {[...groups.entries()].map(([sid, list]) => (
        <SourceSection key={sid} catalog={catalog} id={sid}>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
            {(list as Mcp[]).map((m) => (
              <div key={m.name + m.detail} className="rounded-lg border border-border p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-medium">{m.name}</span>
                  <Badge variant="secondary" className="font-normal">{m.type}</Badge>
                  {m.scope && <Badge variant="outline" className="font-normal">{m.scope}</Badge>}
                  <Badge variant="outline" className={cn("font-normal", !m.enabled && "opacity-50")}>
                    {m.enabled ? "ativo" : "inativo"}
                  </Badge>
                </div>
                {m.detail && <div className="mt-1.5 truncate font-mono text-[11px] text-muted-foreground">{m.detail}</div>}
              </div>
            ))}
          </div>
        </SourceSection>
...
export function PluginsView() {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">{catalog.plugins.length} plugins/marketplaces</p>
      {[...groups.entries()].map(([sid, list]) => (
        <SourceSection key={sid} catalog={catalog} id={sid}>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
            {(list as Plugin[]).map((p) => (
              <div key={p.name + p.detail} className="rounded-lg border border-border p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-medium">{p.name}</span>
                  {p.scope && <Badge variant="outline" className="font-normal">{p.scope}</Badge>}
                  <Badge variant="outline" className={cn("font-normal", !p.enabled && "opacity-50")}>
                    {p.enabled ? "ativo" : "inativo"}
                  </Badge>
                </div>
                {p.detail && <div className="mt-1.5 truncate text-[11px] text-muted-foreground">{p.detail}</div>}
              </div>
            ))}
          </div>
        </SourceSection>
  ...
  const highlight = (text: string) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === q.toLowerCase()
        ? <b key={i} className="text-amber-400">{part}</b>
        : part,
    )
  }
  if (q.length < 2) return <p className="text-sm text-muted-foreground">Digite ao menos 2 letras para buscar.</p>
      ...
      {[...groups.entries()].map(([sid, list]) => (
        <Section key={sid} title={<SourceBadge source={catalog?.sources.find((s) => s.id === sid)} />}>
          {(list as SearchResult[]).map((r) => (
            <div key={r.s + r.r} className="border-b border-border py-2 last:border-0">
              <button
                onClick={() => open(r)}
                className="flex w-full items-center gap-2 rounded-md px-2 py-1 text-left font-mono text-xs hover:bg-accent"
              >
                <span className="truncate">{r.n}</span>
                <span className="ml-auto truncate text-[10.5px] text-muted-foreground">{r.r}</span>
              </button>
              {r.matches.map((m) => (
                <div key={m.n} className="ml-5 font-mono text-[11px] text-muted-foreground">
    ...
    <div className="relative flex-1">
      ...
      {value && (
        <button
          ...
          onClick={() => { setValue(""); setParams({}); navigate(-1) }}
        >
          <X className="size-4" />
        </button>
      )}
